...
3. *Optionally*, discards fields other than genotype `GT` and read depth `DP` from cells indicating no reads supporting an alternate allele `AD=*,0`. Also rounds `DP` down to a power of two in these cells.

That's all! The first two ideas are lossless, while the optional lossy transformation increases compression for little practical tradeoff.

Drawbacks:

1. Complicates random access into the matrix on either dimension
2. Breaking change to VCF
3. Technique isn't readily applicable to BCF
